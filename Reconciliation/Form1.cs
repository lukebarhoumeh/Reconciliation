    ...
    public partial class Form1 : Form
    {
        ...
        private async void btnCompare_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rbExternal.Checked == true)
                {
                    ...
                    {
                        return FindMismatchDetails(_sixDotOneDataView.Table, _microsoftDataView.Table).AsDataView();
                    });
        ...
        private DataTable FindMismatchDetails(DataTable sixdotOneDataTable, DataTable microsoftDataTable)
        {
            ...
            try
            {
                ...
                string[] columnsMspHubBilling = ["CustomerName", "CustomerDomainName", "ProductId", "ProductName", "SkuId", "ChargeType", "SubscriptionStartDate", "SubscriptionEndDate", "EffectiveUnitPrice", "Quantity", "OrderDate", "Term", "BillingCycle", "MSRPPrice", "EffectiveMSRPPrice", "ChargeStartDate", "ChargeEndDate", "PartnerEffectiveUnitPrice", "AdminDiscountPercentage", "PartnerDiscountPercentage", "Total"];
                string[] columnsMicrosoft = ["CustomerName", "CustomerDomainName", "ProductId", "ProductName", "SkuId", "ChargeType", "SubscriptionStartDate", "SubscriptionEndDate", "EffectiveUnitPrice", "Quantity", "OrderDate", "Term", "BillingCycle", "ChargeStartDate", "ChargeEndDate", "Total"];

                var dateColumns = new HashSet<string> { "ValidFrom", "ValidTo", "SubscriptionStartDate", "SubscriptionEndDate", "OrderDate", "ChargeStartDate", "ChargeEndDate" };
                var decimalColumns = new HashSet<string> { "EffectiveUnitPrice", "Quantity", "MSRPPrice", "PartnerEffectiveUnitPrice", "AdminDiscountPercentage", "PartnerDiscountPercentage" };
                ...
                foreach (var column in columnsMicrosoft.Union(columnsMspHubBilling))
                {
                    ...
                    if (decimalColumns.Contains(column))
                        columnType = typeof(decimal); // Assign Decimal type
                ...
                // mismatchTable.Columns.Add(new DataColumn("CalculatedEffMSRP", typeof(decimal)));
                mismatchTable.Columns.Add(new DataColumn("EffectiveMSRPCheck", typeof(string)));
                // mismatchTable.Columns.Add(new DataColumn("CalculatedEffUnitPrice", typeof(decimal)));
                mismatchTable.Columns.Add(new DataColumn("EffectiveUnitPriceCheck", typeof(string)));

                var keyOrder = new[] { "ProductId", "SkuId", "ChargeType", "Quantity", "SubscriptionStartDate", "SubscriptionEndDate", "Term", "BillingCycle", };
                //var mustMatchKeys = new[] { "ProductId", "SkuId", "ChargeType", "Quantity", "SubscriptionStartDate", "SubscriptionEndDate", "Term", "BillingCycle", };
                ...
                try
                {
                    ...
                    groupedMicrosoftData = filteredMicrosoftDataTable.AsEnumerable()
                        .Where(row =>
                            !string.IsNullOrWhiteSpace(SafeGetString(row, "CustomerName")) &&
                ...
                {
                    AppendLog($"Error while filtering and grouping data: {ex.Message}");
                    return mismatchTable;
                }
                foreach (var microsoftGroup in groupedMicrosoftData)
                {
                    if (!microsoftGroup.Rows.Any()) continue;

                    // Extract CustomerName and CustomerDomainName from Key
                    ...
                    // Find a matching group in groupedSixdotOneData where either CustomerName or CustomerDomainName matches
                    ...
                    if (sixdotOneGroup != null)
                    {
                        foreach (var microsoftRow in microsoftGroup.Rows)
                        {
                            try
                            {
                                ...
                                else // Matching records further validation
                                {
                            ...
                            {
                                AppendLog($"Error processing Microsoft row: {ex.Message}");
                            }
                        }
                        ...
                        foreach (var sixdotOneRow in sixdotOneGroup.Rows)
                        {
                            try
                            {
                                ...
                                if (matchingMicrosoftRow != null)
                                {
                                    ...
                                    {
                                        newRowRequired = true;
                                        // newRow["CalculatedEffMSRP"] = Math.Round(calculatedEffMSRP, 8);
                                    ...
                                    {
                                        newRowRequired = true;
                                        //newRow["CalculatedEffUnitPrice"] = Math.Round(calculatedEffUnitPrice, 8);
                            ...
                            {
                                AppendLog($"Error processing MSP Hub row: {ex.Message}");
                            }
                        }
                    }
                    else
                    {
                        foreach (var microsoftRow in microsoftGroup.Rows)
                        {
                            ...
                            {
                                AppendLog($"Error adding missing Microsoft row: {ex.Message}");
                            }
                        }
                    }
                }
                ...
                {
                    if (!sixdotOneGroup.Rows.Any()) continue;

                    string customerKey = sixdotOneGroup.Key; // "CustomerName - CustomerDomainName"
                    string[] keyParts = customerKey.Split(" - "); // Extract CustomerName and CustomerDomainName
                    string customerName = keyParts.Length > 0 ? keyParts[0] : string.Empty;
                    string customerDomain = keyParts.Length > 1 ? keyParts[1] : string.Empty;
                    ...
                    if (microsoftGroup == null)
                    {
                        foreach (var sixdotOneRow in sixdotOneGroup.Rows)
                        {
                            ...
                            {
                                AppendLog($"Error adding missing MSP Hub row: {ex.Message}");
                            }
                        }
                    }
                }
            ...
            {
                AppendLog($"Critical error in FindMismatchDetails: {ex.Message}");
                return new DataTable(); // Return empty table if there's a critical failure
            }
        }

        static decimal CalculatePercentage(decimal value, decimal percent)
        {
            return (value * percent) / 100;
        }

        private void LogComparisonStart()
        {
            AppendLog($"******************** Comparison of Microsoft and MSP Hub Invoice Run ********************");
            AppendLog($"\nComparison of Microsoft and MSP Hub Invoice started at: {DateTime.Now}");
        }
        private void ProcessMismatchSummary(DataTable mismatchTable, List<GroupedCustomerData> groupedMicrosoftData, List<GroupedCustomerData> groupedSixdotOneData, Stopwatch stopwatch)
        {
            ...
            AppendLog($"\nTotal Line Items Processed (Microsoft): {microsoftRowCount}");
            AppendLog($"\nTotal Line Items Processed (MSP Hub): {sixdotOneRowCount}");
            ...
            AppendLog($"\nTotal Customers Processed for Mismatch (Microsoft): {uniqueMicrosoftCustomers}");
            AppendLog($"\nTotal Customers Processed for Mismatch (MSP Hub): {uniqueSixdotOneCustomers}");
        ...
        private void AddSeparatorRow(DataTable mismatchTable)
        {
            if (mismatchTable.Rows.Count == 0 || mismatchTable.Rows[mismatchTable.Rows.Count - 1].ItemArray.All(value => value.ToString() == "-----"))
                return;

            DataRow separatorRow = mismatchTable.NewRow();
            foreach (DataColumn col in mismatchTable.Columns)
            {
                separatorRow[col.ColumnName] = "-----";  // Fill each column with dashes
            }
            mismatchTable.Rows.Add(separatorRow);
        }
